...
> **Implemented (ADR-027):** `portal/sw.js` precaches the entire platform shell plus every known campaign's complete `src`/`generated` content on first load — not just images/icons/fonts, but every JSON file a mission, Parent Mode or the workbook could ever fetch — using a cache-first strategy with a network fallback. The file list is generated by `scripts/generate_offline_manifest.py` into each campaign's own `generated/offline-manifest.json`, re-run whenever campaign content changes, alongside a manual `CACHE_VERSION` bump in `sw.js` so returning devices actually pick up the update.
