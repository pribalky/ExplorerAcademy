...
- [x] Tablet — **fully fixed post-Phase-11.** `layout.css`/`components.css` (previously empty Milestone-1.1 placeholders) now give every interactive control — buttons, links, `<summary>` disclosures, form fields, and the labels wrapping radio/checkbox inputs — a minimum 44px touch target, plus generous spacing between list items and a wrapping primary nav. Verified directly: every measured interactive element on Home, Campaign Select, a mission page, Settings and the Explorer selector is ≥44px tall at 768×1024, with zero horizontal overflow.

- [x] Mobile — **fully fixed post-Phase-11**, same fix as Tablet. Verified identically at 375×667: zero interactive elements under 44px, zero horizontal overflow, confirmed even with x-large font + high-contrast accessibility settings combined.
...
- [x] Automated regression coverage — **added post-Phase-11.** Every check above (and every later milestone's verification) had been a throwaway Playwright script re-derived from scratch each session. A checked-in `tests/` suite (`python3 tests/run_all.py`) now covers broken links, multi-child profile isolation, mission/reflection Save State, Settings/accessibility, Save Export/Import, offline caching, Parent Mode, and touch targets — 8 test files, all passing, proven to actually catch a regression (a deliberately injected bug was confirmed to fail 6 of 8 test files before being reverted). See `tests/README.md`.
...
**The user then asked for Save Export/Import next, so a parent could move an Explorer between devices "smoothly."** Clarified upfront that this would be a manual downloaded-and-re-uploaded file, not automatic cross-device sync (ADR-026 already excludes real sync from this architecture). Added thin `exportSave()`/`importSave()` wrappers in `storage.js`, and `exportProfile(childId)`/`importProfile(exportedData)` in `explorer-profiles.js` — export bundles the profile's identity fields (name, avatar, PIN hash, streak) plus that child's full save into one versioned JSON object; import validates the file, rejects a display-name collision against an existing local profile, and always mints a fresh local id rather than reusing the file's id. Only the PIN's hash travels in the file, never the plaintext PIN, so the same PIN keeps working after import without ever being written to disk. Settings gained a "Backup & Transfer" section with a "Download Backup File" button (`Blob` + `<a download>`); Home's Explorer selector gained a collapsed "+ Import Explorer" file-picker form. ADR-028 records the decision, explicitly distinguishing this from the sync ADR-026 excludes. Verified end-to-end across separate browser contexts standing in for separate devices: export → fresh-device import reproduces the Explorer exactly; the original PIN still works after import; a name collision is rejected with a clear message and no duplicate created; a malformed or foreign JSON file is rejected gracefully; re-importing the same file twice doesn't create a second profile (correctly hits the same collision rejection); and a full regression pass shows zero impact on any existing flow.

Phase 12 (Campaign Release) remains the only original-roadmap phase not yet started. Awaiting user direction.

**The user then asked to close the Tablet/Mobile touch-target gap next**, since the platform is likely to be used on mobile/laptop. `layout.css` and `components.css` had been empty Milestone-1.1 placeholders since Phase 1 — no responsive or touch-aware CSS existed anywhere. Added a `box-sizing: border-box` reset and a wrapping, gapped primary nav to `layout.css`; added a ~44px minimum touch target (per the ~44px guideline 601_HTML_ARCHITECTURE.md's Responsive Behaviour section calls for) to every interactive control in `components.css` — buttons, text/file inputs, `<summary>` disclosure triggers, in-content links, and the `<label>` elements that wrap the platform's radio/checkbox inputs (native checkboxes/radios render far under 44px on their own, but every one in the platform is already wrapped in a label the child can tap anywhere on). Targeted plain HTML elements rather than a new class system, since every interactive control across `router.js`/`parent-mode.js` is built with plain DOM APIs, not a shared component function — so no JavaScript changed, only CSS. Verified directly via headless Chromium at Tablet (768×1024), Mobile (375×667) and Desktop (1440×900): zero interactive elements measured under 44px across Home, Campaign Select, a mission page, Settings, the Explorer selector (including the new Import Explorer form), and Parent Mode's picker/PIN gate/dashboard; zero horizontal overflow at any size, including with x-large font + high-contrast accessibility settings stacked on top; all 21 missions still render correctly with zero console/page errors; full regression pass (multi-child isolation, Backup & Transfer, Discovery Log) unaffected.

Phase 12 (Campaign Release) remains the only original-roadmap phase not yet started. Awaiting user direction.

**Asked for a high-value recommendation beyond the known roadmap, the two picks offered were a checked-in automated test suite and the README's Explorer's Field Journal differentiator idea; the user said yes to both, test suite first.** Every verification performed across this entire session — Phase 11, offline caching, Save Export/Import, the touch-target CSS fix — had been a throwaway Playwright script written in `/tmp` and discarded, so nothing regression-tested the platform between sessions. Added a checked-in `tests/` directory: `helpers.py` (spins up its own `python3 -m http.server` against `portal/` on a free port per test file, launches this environment's Chromium), 8 standalone `test_*.py` files ported from this session's own proven ad-hoc scripts (broken links across all 21 missions + Parent Mode, multi-child profile isolation, mission/reflection Save State, Settings + accessibility, Save Export/Import's full round-trip, offline caching, Parent Mode's picker/PIN/dashboard, and the touch-target sweep), and `run_all.py` to discover and aggregate them. Deliberately no pytest (not installed, not used anywhere else in the repo) — plain scripts with a `run() -> bool`, matching the existing `scripts/build_workbook.py`/`generate_offline_manifest.py` pattern of checked-in, dependency-light Python tooling. `tests/README.md` documents how to run it (`python3 tests/run_all.py`, nothing new to install). Caught one real bug in a *test* (not the app) while first running the suite — `test_missions.py` checked for the active Explorer's name on the mission page after a reload, but that page never displays it (only Home's dashboard does) — fixed by checking Home instead. Proved the suite has real teeth, not just trivial passes: deliberately renamed the "Create Explorer" button's text in `router.js`, re-ran the suite, confirmed 6 of 8 test files correctly failed, then reverted the change and confirmed all 8 pass again. `CLAUDE.md`'s Repository Layout list now includes `tests/`.

Phase 12 (Campaign Release) and the Explorer's Field Journal remain the next candidates. Awaiting user direction.
...
**Before starting the second campaign, the user asked to make sure Campaign 1 genuinely works — and, running it locally for the first time via the new guide, immediately found a real bug: "campaign1 shows only summary, no missions."** Confirmed as a genuine, significant, pre-existing gap, not a local setup issue: the Campaign Overview page never rendered any mission links — the only ways into a mission were "Continue Mission" (needs an existing session) or typing a URL by hand. This slipped through every prior testing pass, including this project's own automated suite, because every check ever written navigated straight to mission URLs directly rather than clicking through the real UI from Campaign Overview. Fixed in `router.js` (a new `renderMissionList()` turns the campaign's mission list into real, correctly-titled links); `tests/test_broken_links.py` extended to click through the actual UI path, and proven to catch the exact regression via a revert-and-restore check. The user's second report — unable to save the session duration in Settings — could not be reproduced in this environment (direct clicks, label-center clicks, and label-padding clicks all correctly select and persist every duration in headless Chromium); awaiting more detail (browser, exact behavior on click) before treating it as confirmed.

Second campaign work remains paused until Campaign 1 is confirmed fully working end-to-end by the user's own local run.

**After pulling the mission-list fix, the user reported the mission list was still empty.** A second real bug, and this one self-inflicted: the fix shipped without bumping `sw.js`'s `CACHE_VERSION`, so a browser that had already installed the service worker before the fix kept serving the old, cached `router.js` indefinitely — exactly the failure mode `sw.js`'s own header comment warns about. Reproduced the user's exact scenario end-to-end (install old code, swap in the fix on disk without a version bump, confirm the bug persists) before fixing, then confirmed a normal reload — no DevTools tricks needed — correctly picks up the fix once `CACHE_VERSION` is bumped to `v3`. Full regression pass: all 8 test files still pass.