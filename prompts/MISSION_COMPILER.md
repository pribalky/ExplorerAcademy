...
504_JSON_SCHEMA.md
501_CAMPAIGN_01.md
portal/campaigns/campaign01/src/missions/mission01.json (reference pattern — see below)

Generate:

missionXX.json (one file per mission, e.g. mission02.json)
...
- Include mission beats: all 8 canonical types (HOOK, MYSTERY, INVESTIGATION, DISCOVERY, CHALLENGE, BREAKTHROUGH, REFLECTION, CLIFFHANGER), each with sequence/narrativeText/trigger, restructured from the mission's own Story Summary/Core Activities/Story Outcome — never inventing new plot facts beyond what 501 states.
- Include all activities (Core, Extension, Rabbit Hole from 501's own lists for that mission) as full embedded objects, not ID references.
...
- Include a parentGuide (id, learningObjectives, discussionPoints, preparation, assessment) — learningObjectives drawn from 501's own Learning Focus list.
...
> **Note (added during the Mission Compiler milestone):** this originally said "Follow 503_DATA_MODEL.md exactly" and "Reference shared campaign entities by ID" without qualifying which entities count as "shared." 503's Mission requires `Beat IDs`/`Activity IDs` — references to separate entities — which conflicts with 504's embedded-object Mission schema that `mission-engine.js`/`activity-engine.js`/`scheduler.js`/`reward-engine.js` already consume.
>
> Decided with the user, re-verifying rather than assuming the Campaign Compiler decision carries over: Activities/Beats/Rewards/Parent Guide are always 1:1 mission-owned in both 503 and the actual campaign content — never shared or reused across missions — so embedding them (504's approach) isn't a structural trade-off, just the natural shape. "Reference shared campaign entities by ID" now means genuinely campaign-level entities only: `storyChapter` and `campaignId`. Nothing else on a Mission should be a bare ID reference.
>
...
- `ACTIVITY-####` — continue sequentially across every mission's activities (Mission 1 used 0001–0007; check the last compiled mission file for the next available number).
- `REWARD-####`, `PARENTGUIDE-####` — one each per mission, same continue-the-sequence rule.

## Duration Tuning

Not arbitrary — durations should make the Scheduler's four bands (30/45/60/90 minutes) behave meaningfully for every mission, following the pattern validated in Mission 1:
...
Before considering a mission done: confirm it loads and validates via `mission-engine.js` (no code changes required), confirm the Scheduler produces the intended Core/Extension/Rabbit Hole split at each duration band, and confirm no `parentGuide` content appears in the rendered Mission page.
